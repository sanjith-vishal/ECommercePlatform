...
class OrderServiceApplicationTests {
    ...
    @Test
    void testGetOrderWithProductDetails() {
        int orderId = 1;
        int userId = 99;

        Order order = new Order(orderId, userId, 300.0, "Addr", "SHIPPED", "PAID");
        List<CartItemWithProductDTO> cartItems = List.of(
                new CartItemWithProductDTO(userId, 101, 2, new ProductDTO(101, "Laptop", "Desc", 150.0,"Electronics")),
                new CartItemWithProductDTO(userId, 102, 1, new ProductDTO(102, "Mouse", "Desc", 50.0,"Electronics"))
        );

        when(orderRepository.findById(orderId)).thenReturn(Optional.of(order));
        when(cartClient.getCartItemsWithProducts(userId)).thenReturn(cartItems);

        OrderWithProductDTO result = orderService.getOrderWithProductDetails(orderId);

        assertEquals(order, result.getOrder());
        assertEquals(2, result.getProducts().size());
        assertEquals("Laptop", result.getProducts().get(0).getName());
    }
}